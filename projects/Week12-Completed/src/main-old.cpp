...
struct DebugGuiState {
	...
	BulletDebugMode PhysicsDebugMode;
};

void DebugDraw(DebugGuiState& state, float& playbackSpeed) {
		...
		if (isDebugWindowOpen) {
			...
			if (ImGui::Button(buttonLabel)) {
				...
				if (!scene->IsPlaying) {
					...
					scene = Scene::FromJson(state.SceneState);
			...
			if (BulletDebugDraw::DrawModeGui("Physics Debug Mode:", state.PhysicsDebugMode)) {
				scene->SetPhysicsDebugDrawMode(state.PhysicsDebugMode);
			}
...
int main2(int argCount, char** arguments) {
	...
	fboDescriptor.Width = windowSize.x;
	fboDescriptor.Height = windowSize.y;
	fboDescriptor.GenerateUnsampled = false;
	fboDescriptor.SampleCount = 1;

	fboDescriptor.RenderTargets[RenderTargetAttachment::Depth] ={ true, RenderTargetType::Depth24 };
	fboDescriptor.RenderTargets[RenderTargetAttachment::Color0] ={ true, RenderTargetType::ColorRgb8 };

	primaryFbo = std::make_shared<Framebuffer>(fboDescriptor);
	...
	DebugGuiState guiState;
	guiState.IsDebugVisible = false;
	guiState.PhysicsDebugMode = BulletDebugMode::None;
	guiState.ScenePath.reserve(256);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		DebugDraw(guiState, playbackSpeed);

		if (InputEngine::GetKeyState(GLFW_KEY_F1) == ButtonState::Pressed) {
			guiState.IsDebugVisible = !guiState.IsDebugVisible;
		}
		...
		primaryFbo->Unbind();
		Framebuffer::Blit(primaryFbo, nullptr);
