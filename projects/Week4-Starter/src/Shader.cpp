...
{
	LOG_ASSERT(count == 1, "Set Uniform for bools only supports setting single values at a time!");
	glProgramUniform1i(location, *value, 1);
}

void Shader::SetUniform(int location, const glm::bvec2* value, int count)
{
	LOG_ASSERT(count == 1, "SetUniform for bools only support setting single values as a time!");
...
int Shader::__GetUniformLocation(const std::string& name)
{
	...
	//If out entry was not found, we call glGetUniform and store it for the next time
