...
template <typename Structure>
void UniformBuffer<Structure>::Update() {
	glNamedBufferSubData(_handle, 0, sizeof(Structure), _rawData);
}

template <typename Structure>
void UniformBuffer<Structure>::SetData(const Structure& data) {
	*((Structure*)_rawData) = data;
	Update();
}

template <typename Structure>
const Structure& UniformBuffer<Structure>::GetData() const {
	return *reinterpret_cast<Structure*>(_rawData);
}

template <typename Structure>
Structure& UniformBuffer<Structure>::GetData() {
	return *reinterpret_cast<Structure*>(_rawData);
}

template <typename Structure>
UniformBuffer<Structure>::UniformBuffer(const Structure& data, BufferUsage usage) :
...
template <typename Structure>
UniformBuffer<Structure>::UniformBuffer(BufferUsage usage) :
	AbstractUniformBuffer(sizeof(Structure), usage)
{ }
