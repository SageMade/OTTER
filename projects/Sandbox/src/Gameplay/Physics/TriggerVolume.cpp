...
void TriggerVolume::PhysicsPostStep(float dt) {
	...
	int numObjects = _ghost->getNumOverlappingObjects();
	...
	for (int i = 0; i < numObjects; ++i) {
		...
		const btCollisionObject *obj = _ghost->getOverlappingObject(i);

		// If it's group matches our mask (since this isn't filtered for us)
		if (obj->getBroadphaseHandle()->m_collisionFilterGroup & _collisionMask) {
				...
				if ((body->getCollisionFlags() & btCollisionObject::CF_STATIC_OBJECT)    == 0 &&
					...
					if (physicsPtr != nullptr) {
						...
						if (it == _currentCollisions.end()) {
							// Iterate over all components, and invoke it's bound callback if it exists
							for (auto& [key, callback] : _enterCallbacks) {
								if (callback) {
									callback(physicsPtr);
								}
							}
	...
	for (auto& weakPtr : _currentCollisions) {
		...
		if (it == thisFrameCollision.end()) {
			// Iterate over all components, and invoke it's bound callback if it exists
			for (auto& [key, callback] : _exitCallbacks) {
				if (callback) {
					callback(weakPtr.lock());
				}
			}
		}
	}
...
void TriggerVolume::SetEnterCallback(const std::shared_ptr<IComponent>& component, TriggerCallback callback) {
	_enterCallbacks[component] = callback;
}

void TriggerVolume::RemoveEnterCallback(const std::shared_ptr<IComponent>& component) {
	_enterCallbacks[component] = nullptr;
}

void TriggerVolume::SetLeaveCallback(const std::shared_ptr<IComponent>& component, TriggerCallback callback) {
	_exitCallbacks[component] = callback;
}

void TriggerVolume::RemoveLeaveCallback(const std::shared_ptr<IComponent>& component) {
	_exitCallbacks[component] = nullptr;
}
