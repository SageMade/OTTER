...
VertexArrayObject::Sptr OptimizedObjLoader::LoadFromFile(const std::string& filename) {
	...
	if (extension == ".obj") {
		MeshBuilder<VertexPosNormTexCol>* mesh = _LoadFromObjFile(filename);
		VertexArrayObject::Sptr result = mesh->Bake();
		delete mesh;
		return result;
	} 
	// Load our fancy binary files
	else if (extension == ".bobj") {
...
void OptimizedObjLoader::ConvertToBinary(const std::string& inFile, const std::string& outFile) {
	...
	MeshBuilder<VertexPosNormTexCol>* mesh = _LoadFromObjFile(inFile);
...
MeshBuilder<VertexPosNormTexCol>* OptimizedObjLoader::_LoadFromObjFile(const std::string& filename) {
	...
	MeshBuilder<VertexPosNormTexCol>* mesh = new MeshBuilder<VertexPosNormTexCol>();
...
VertexArrayObject::Sptr OptimizedObjLoader::_LoadFromBinFile(const std::string& filename) {
	...
	if (header.Version == 0x01) {
		...
		size_t requiredBytes =
			...
			(header.VertexStride * header.NumVertices) +
